...
from data.vqarad_dataset import VQA_dataset,Slake_dataset,VQA_ordinary_dataset, VQA_slake_test_dataset, VQA_Instruction_dataset
from torchvision import transforms
from torch.nn.parallel import DataParallel

from peft import get_peft_model, get_peft_config, get_peft_model_state_dict, TaskType, IA3Config
...
def main(args, config):
    ...
    ia3_config = IA3Config(task_type=TaskType.CAUSAL_LM,
                        target_modules=["key", "value", "output.dense"],
                        inference_mode=False, 
                        feedforward_modules=["output.dense"])
    
    model.text_decoder = get_peft_model(model.text_decoder, ia3_config)
    print('text_decoder参数:')
    ...
    optimizer = torch.optim.AdamW(params=model.parameters(), lr=config['init_lr'], weight_decay=config['weight_decay']) ############# 需要过滤冻结参数吗？
    ...
    for epoch in range(0, config['max_epoch']):
        print('max_epoch: ', str(config['max_epoch']))
    # for epoch in range(start_epoch, config['max_epoch']):
        ...
        if utils.is_main_process():     
            ...
            # 存储ia3权重
            peft_model_id = os.path.join(args.output_dir, f'IA3_decoder_checkpoint_{epoch}')
            model.text_decoder.save_pretrained(peft_model_id)

            torch.save(model.text_decoder, os.path.join(args.output_dir, 'pth_de_checkpoint_%02d.pth'%epoch))
