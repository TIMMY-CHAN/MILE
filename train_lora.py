...
def main(args, config):
    ...
    # for epoch in range(start_epoch, config['max_epoch']):
        ...
        if utils.is_main_process():     
            ...
            # #  没有微调，预训练模型参数没有动，不需要存checkpoint
            # 存储checkpoint模型
            torch.save(save_obj, os.path.join(args.output_dir, 'checkpoint_%02d.pth'%epoch))
            

            # 存储lora_text_encoder权重
            peft_model_id = os.path.join(args.output_dir, f'lora_encoder_checkpoint_{epoch}')
            model.text_encoder.save_pretrained(peft_model_id)
            
            # 存储lora_text_decoder权重
            peft_model_id = os.path.join(args.output_dir, f'lora_decoder_checkpoint_{epoch}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='/Dataset1/cjw/pretrained/VQA_PEFT/Instruction/Lora/task_b1_r4')
